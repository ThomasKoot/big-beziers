import { basePointRadius, intermediateRadiusRatio, endPointRatio } from './constants'

export function findBezierPaths(points, phase) {
    ...
    let p = points.map(e => [e[0], e[1]]);
    let paths = [];
    while (p.length >= 2) {
        paths.push(calculatePaths(p, basePointRadius * ((p.length === points.length) ? 1 : intermediateRadiusRatio)));
        p = findNewPoints(p, phase)
    }
    paths.push(calculatePaths(p, basePointRadius * endPointRatio))
    return paths;
}
